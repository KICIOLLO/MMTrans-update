function cal_mag_trans_RTP_ext(str_open,I0_ang,D0_ang,I1_ang,D1_ang,DirRes,SavFor,ext_para)

% 函数说明： 磁场转换函数 - 化到地磁极 (Reduct-to-the-pole, RTP)
...
% 开始进行转换操作
Delta2RTP(Mat,Dx,Dy,I0_ang,D0_ang,I1_ang,D1_ang,Per,ext_para);
FulNamRTP=[DirRes FilSep Nam '_RTP'];
load RTP;
RTP=RecMat(RTP,FulNamRTP,RowGap,ColGap,Str,SavFor);
delete('RTP.mat');
clear RTP;
...
%=======================================================================
function Delta2RTP(data,Dx,Dy,Incl,Decl,Incl1,Decl1,Per,ext_para)
...
[FX11,FZ11]=CalInvB_RTP(nrhP,nchP,s,t,p,s1,t1,p1,kx0,ky0);
% Z component filter
FZ11=FZ11.*FX11; 
...
[FX21,FZ21]=CalInvBR_RTP(nrhP,nchM,s,t,p,s1,t1,p1,kx0,ky0);
% Z component filter 
FZ21=FZ21.*FX21; 
...
for ii=1:NumRow
    for jj=1:NumCol
        if ext_para == 1
    		RTP(ii,jj)=Z1(ii+limrow1,jj+limcol1);
    	elseif ext_para == 2
        	RTP(ii,jj)=Z1(ii+ext4,jj+ext1);
        end
    end
end
clear Z1;
...
%=======================================================================
function [InvB,K]=CalInvB_RTP(nx,ny,s,t,p,s1,t1,p1,kx0,ky0)
%=======================================================================
...
for x=1:nx
   ...
   for y=1:ny
      ...
      k=sqrt(k2); 
      K(x,y)=k2;
      B(x,y)=(k*s+i*(kx*t+ky*p))*(k*s1+i*(kx*t1+ky*p1));
   end % for y=1:ny    
end % for x=1:nx   
B(1,1)=1;
InvB=1./B; 
...
%=======================================================================
function [InvB,K]=CalInvBR_RTP(nx,ny,s,t,p,s1,t1,p1,kx0,ky0)
%=======================================================================
