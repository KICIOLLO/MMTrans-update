...
%           trans_xyz      参数，用以确定是进行哪种梯度变换，形式为[a b c d]
%				        a == 1     x方向导数
%               b == 1     y方向导数
%               c == 1     z方向导数
%               d == 1     total gradient
...
if para_xyz(4) ~= 1
	...
	FulNamXG=[DirRes FilSep Nam '_XGrad'];
	load dataX;
	Gradx=RecMat(dataX,FulNamXG,RowGap,ColGap,Str,SavFor);
	YGrad(Mat,Dx,Dy,Per,ext_para);
	FulNamYG=[DirRes FilSep Nam '_YGrad'];
	load dataY;
	Grady=RecMat(dataY,FulNamYG,RowGap,ColGap,Str,SavFor);
	ZGrad(Mat,Dx,Dy,Per,ext_para);
	FulNamZG=[DirRes FilSep Nam '_ZGrad'];
	load dataZ;
	Gradz=RecMat(dataZ,FulNamZG,RowGap,ColGap,Str,SavFor);

	dataXYZ = sqrt(dataX.*dataX + dataY.*dataY + dataZ.*dataZ);
	FulNamXYZG=[DirRes FilSep Nam '_Gradxyz'];
	Gradxyz=RecMat(dataXYZ,FulNamXYZG,RowGap,ColGap,Str,SavFor);
	delete('dataX.mat');
	delete('dataY.mat');
	delete('dataZ.mat');
end
